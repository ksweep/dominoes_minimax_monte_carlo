...
void MonteCarloDataGenerator::_updateChildCounts(const bool isMaxPlayer, const GameFoundation& gameFoundation, GameState& workState, IntVector& childCounts, IntPair& maxIndices) {
    ...
    if (bestChildIndex != maxIndices.first) {
        if (childCounts[bestChildIndex] > childCounts[maxIndices.first]) {
            maxIndices.second = maxIndices.first;
            maxIndices.first = bestChildIndex;
        } else if (childCounts[bestChildIndex] > childCounts[maxIndices.second]) {
            maxIndices.second = bestChildIndex;
        }
    }
}
...
MonteCarloData MonteCarloDataGenerator::generateData(const Game &game, const GameState &initialState, const int numberOfSamples) {
    ...
    while (true) {
        ...
        GameState workState = GameState::stateForMonteCarloSampling(mostRecentState);

        // sample random hands to find best move
        IntVector childCounts(game.gameFoundation.handSize, 0);
        IntPair maxIndices = std::make_pair(0, 1);

        for (int currentSampleNumber = 0; currentSampleNumber < samplesToUse; currentSampleNumber++) {
            ...
            _updateChildCounts(isMaxPlayer, game.gameFoundation, workState, childCounts, maxIndices);

            // if the count of this child can't be exceeded by any other child, no need to keep sampling
            const int remainingSamples = currentSampleNumber + 1;
            if (childCounts[maxIndices.first] > childCounts[maxIndices.second] + (samplesToUse - remainingSamples)) {
                break;
            }
